...
class TestCustomerService:
    ...
    def test_create_customer_success(self, customer_service, mock_uow, customer):
        # Arrange
        data = CreateCustomer(name="John Doe", email="john@example.com")
        
        mock_uow.customer_repo.create.return_value = customer
        
        # Act
        result = customer_service.create(data)

        # Assert
        mock_uow.customer_repo.create.assert_called_once_with(data)
        assert result.id == 1
        assert result.name == "John Doe"
        assert result.email == "john@example.com"

    def test_create_customer_raises_when_email_taken(self, customer_service, mock_uow):
        # Arrange
        data = CreateCustomer(name="John Doe", email="taken@example.com")
        
        mock_uow.customer_repo.create.side_effect = EmailAlreadyTakenError("taken@example.com")
